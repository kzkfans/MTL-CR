...
from .cpgs.cpg_parser import generate_single_cpg_nl
import enums
...
def parse_json_file_with_cpg(file, lang):
    """
    Parse a dataset file where each line is a json string representing a sample.
    ...
    with open(file, encoding='utf-8') as f:
        for line in f.readlines():
            ...
            cpg_dot = ''.join(data['cpg'])
            cpgs_dot.append(cpg_dot)
    ...
    return sources, codes, names, codes_wo_name, docs, method_names, cpgs_dot
...
def load_pre_train_dataset_with_cpg(file, lang):
    """
    Load json dataset from given file.
    ...
    if lang in [enums.LANG_JAVA, enums.LANG_PYTHON, enums.LANG_GO,
                enums.LANG_JAVASCRIPT, enums.LANG_PHP, enums.LANG_RUBY]:
        sources, codes, names, codes_wo_name, docs, method_names, cpgs_dot = parse_json_file_with_cpg(file, lang=lang)
        return sources, codes, names, codes_wo_name, docs, method_names, cpgs_dot
...
#     all_only_names = []
#     all_docs = []
#     for file in os.listdir(dataset_dir):  # language
#
#         path = os.path.join(dataset_dir, file)  # language folder
...
#             continue
#
#         dataset_files = iter_pre_train_dataset_files(path, lang=lang)  # .jsonl文件的路径 (每个语言三个：train，test，valid)
#
...
#                 # TODO: 读新数据集时多一个cpg
#                 sources, codes, names, codes_wo_name, docs, method_names, cpgs_dot = load_pre_train_dataset_with_cpg(
...
#                 cnt = 0
#                 # TODO: 多一个cpg
#                 for source, code, name, code_wo_name, method_name, cpg_dot in tqdm(
#                         zip(sources, codes, names, codes_wo_name, method_names, cpgs_dot),
...
#                         # print("ast:", ast)
#
#                         cpg_seq, nl, nl_wo_name = generate_single_cpg_nl(source=source,
#                                                                          lang=lang,
#                                                                          name=name,
#                                                                          cpg_dot=cpg_dot,
#                                                                          replace_method_name=True
...
#
#     assert len(languages) == len(all_sources) == len(all_codes) == len(all_codes_wo_name) == len(all_cpgs) == \
#            len(all_names) == len(all_names_wo_name) == len(all_only_names)
#     return paths, languages, all_sources, all_codes, all_cpgs, all_names, all_codes_wo_name, all_names_wo_name, \
#         all_only_names, all_docs
...
def parse_for_summarization(source_path, code_path, nl_path, lang):
    ...
    # 前三行测试
    # sources, codes, nls = sources[:3], codes[:3], nls[:3]
    ...
    for source, code, nl in tqdm(zip(sources, codes, nls), desc='Parsing', leave=False, total=len(sources)):
        try:
            ...
            # print(re.search("\\\\.",source) is not None)
            # print(repr(source))
            # source = source.replace('\\n', '\n')
            # print(repr(source))
            # print("source:", source[:20], "\n", "lang:", lang)
            # ast, name = generate_single_cpg_nl(source=source, lang=lang)
